...
class common extends control
{
    ...
    public static function printOrderLink($fieldName, $orderBy, $vars, $label, $module = '', $method = '')
    {
        ...
        $order_way = substr($orderBy, strrpos($orderBy, '_')+1);//@Green has changed the codes;
        if($order == $fieldName)
        {
            if(isset($order_way) and $order_way == 'asc')
            {
                $orderBy   = "{$order}_desc";
    ...
    public static function printIcon($module, $method, $vars = '', $object = '', $type = 'button', $icon = '', $target = '', $extraClass = '', $onlyBody = false, $misc = '')
    {
        ...
        //@Green delete lines
        if(strtolower($module) == 'resources'      and strtolower($method) == 'tostory')    ($module = 'story') and ($method = 'create');
        if(!common::hasPriv($module, $method)) return false;
        ...
        {
            $title = $method == 'student' ? $lang->$module->$method->common : $lang->$module->$method;
        }
        if($icon == 'toStory')   $title  = $lang->resources->toStory;
        //@Green delete a line.
        ...
        if(strpos(',edit,copy,student,export,delete,', ",$method,") !== false) $module = 'common';
     ...
     * @return void
     */
    static public function printRPN($backLink, $preAndNext = '', $type = 'view')
    {
        global $lang, $app;
        if(isonlybody()) return false;
     ...
     */
    static public function printRPNs($backLink, $preAndNext = '', $group = null, $index = 0)
    {
        global $lang, $app;
        if(isonlybody()) return false;
        ...
        foreach($new as $key => $value)
        {
            ...
            { 
                ...
                if(substr_count($value, "\n") > 1     or 
                   ...
                   strpos('name,title,desc,spec,steps,content,digest,verify,student', strtolower($key)) !== false)
                {
